...
function mergeObjects(objects) {
  ...
  objects.forEach((obj) => {
    Object.entries(obj).forEach((element) => {
      result[element[0]] = (result[element[0]] || 0) + element[1];
    });
  });
  return result;
}
...
function compareObjects(obj1, obj2) {
  let result = true;
  Object.entries(obj1).forEach((element) => {
...
function sellTickets(queue) {
  ...
  for (let i = 0; i < queue.length; i += 1) {
    if (queue[i] === ticketCost) {
      remain += 25;
    } else if (queue[i] - ticketCost <= remain) {
      remain -= queue[i] - ticketCost;
    } else {
      return false;
    }
  }
...
function sortCitiesArray(arr) {
  ...
  return array.sort().map((el) => JSON.parse(el));
}
